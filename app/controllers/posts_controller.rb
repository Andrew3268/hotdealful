class PostsController < ApplicationController
  ...
  def index
    @pagy, @posts = pagy(Post.all.order("created_at DESC"), items: 60)
    if params[:search]
  ...
  def hashtags
    ...
    @pagy, @posts = pagy(tag.posts.order("created_at DESC"), items: 60)
  ...
  private
    ...
    def post_params
      params.require(:post).permit(:title, :link, :image, :source, :hashtag, :rating, :review_count, :is_price, :was_price,
                                   :pct, :description, :promocode, :spare_41, :spare_42, :spare_43)
    end
end
